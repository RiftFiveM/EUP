...
AddEventHandler('FvMain:Client:OnPlayerLoaded', function()
    FvMain.Functions.TriggerCallback('fvm-drugs:server:RequestConfig', function(DealerConfig)
                            ...
                            if IsControlJustPressed(0, Keys["G"]) then
                                if dealer["name"] == "Mystery man" then
                                    ...
                                    if player ~= -1 and distance < 5.0 then
                                        ...
                                        FvMain.Functions.Progressbar("hospital_revive", "Put person back on their feet..", 5000, false, true, {
                                            ...
                                            FvMain.Functions.Notify("You have helped a person!")
                                            TriggerServerEvent("hospital:server:RevivePlayer", playerId, true)
                                        end, function() -- Cancel
                                            isHealingPerson = false
                                            StopAnimTask(PlayerPedId(), healAnimDict, "exit", 1.0)
                                            FvMain.Functions.Notify("Failed!", "error")
                                        end)
                                    else
                                        FvMain.Functions.Notify("There is no one nearby..", "error")
                                    end
                                else
...
function buyDealerStuff()
    ...
    for k, v in pairs(Config.Dealers[currentDealer]["products"]) do
        if FvMain.Functions.GetPlayerData().metadata["dealerrep"] >= Config.Dealers[currentDealer]["products"][k].minrep then
...
function knockDoorAnim(home)
    ...
    if home then
        ...
        knockingDoor = false
        Citizen.Wait(1000)
        FvMain.Functions.Notify('It looks like no one is home..', 'error', 3500)
    end
end

RegisterNetEvent('fvm-drugs:client:updateDealerItems')
AddEventHandler('fvm-drugs:client:updateDealerItems', function(itemData, amount)
    TriggerServerEvent('fvm-drugs:server:updateDealerItems', itemData, amount, currentDealer)
end)

RegisterNetEvent('fvm-drugs:client:setDealerItems')
AddEventHandler('fvm-drugs:client:setDealerItems', function(itemData, amount, dealer)
...
function requestDelivery()
    ...
    TriggerServerEvent('fvm-drugs:server:giveDeliveryItems', amount)
    SetTimeout(2000, function()
        TriggerServerEvent('fvm-phone:server:sendNewMail', {
            sender = Config.Dealers[currentDealer]["name"],
            subject = "Delivery Location",
            message = "Here is all info about the delivery, <br>Items: <br> "..amount.."x "..FvMain.Shared.Items[waitingDelivery["itemData"]["item"]]["label"].."<br><br> be on time!!",
            button = {
                enabled = true,
                buttonEvent = "fvm-drugs:client:setLocation",
...
function randomDeliveryItemOnRep()
    local ped = PlayerPedId()
    local myRep = FvMain.Functions.GetPlayerData().metadata["dealerrep"]
...
function setMapBlip(x, y)
    SetNewWaypoint(x, y)
    FvMain.Functions.Notify('The route to the delivery location has been set on your map.', 'success');
end

RegisterNetEvent('fvm-drugs:client:setLocation')
AddEventHandler('fvm-drugs:client:setLocation', function(locationData)
    if activeDelivery == nil then
        activeDelivery = locationData
    else
        setMapBlip(activeDelivery["coords"]["x"], activeDelivery["coords"]["y"])
        FvMain.Functions.Notify('You still have an open delivery...')
        return
    end
        ...
        while true do
            ...
            if activeDelivery ~= nil then
                ...
                if dist < 15 then
                    ...
                    if dist < 1.5 then
                        DrawText3D(activeDelivery["coords"]["x"], activeDelivery["coords"]["y"], activeDelivery["coords"]["z"], '[E] '..activeDelivery["amount"]..'x '..FvMain.Shared.Items[activeDelivery["itemData"]["item"]]["label"]..' deliver.')
...
function deliverStuff(activeDelivery)
    if deliveryTimeout > 0 then
        ...
        checkPedDistance()
        FvMain.Functions.Progressbar("work_dropbox", "Delivering products..", 3500, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
        }, {}, {}, {}, function() -- Done
            TriggerServerEvent('fvm-drugs:server:succesDelivery', activeDelivery, true)
        end, function() -- Cancel
            ClearPedTasks(PlayerPedId())
            FvMain.Functions.Notify("Canceled..", "error")
        end)
    else
        TriggerServerEvent('fvm-drugs:server:succesDelivery', activeDelivery, false)
    end
    deliveryTimeout = 0
end
...
function doPoliceAlert()
    ...
    TriggerServerEvent('fvm-drugs:server:callCops', streetLabel, pos)
end

RegisterNetEvent('fvm-drugs:client:robberyCall')
AddEventHandler('fvm-drugs:client:robberyCall', function(msg, streetLabel, coords)
...
RegisterNetEvent('fvm-drugs:client:sendDeliveryMail')
AddEventHandler('fvm-drugs:client:sendDeliveryMail', function(type, deliveryData)
    if type == 'perfect' then
        TriggerServerEvent('fvm-phone:server:sendNewMail', {
            sender = Config.Dealers[deliveryData["dealer"]]["name"],
            subject = "Levering",
            message = "You did good work, hope to see you again ;)<br><br>Regards, "..Config.Dealers[deliveryData["dealer"]]["name"]
        })
...
RegisterNetEvent('fvm-drugs:client:CreateDealer')
AddEventHandler('fvm-drugs:client:CreateDealer', function(dealerName, minTime, maxTime)
    local ped = PlayerPedId()
    ...
    TriggerServerEvent('fvm-drugs:server:CreateDealer', DealerData)
end)

RegisterNetEvent('fvm-drugs:client:RefreshDealers')
AddEventHandler('fvm-drugs:client:RefreshDealers', function(DealerData)
    Config.Dealers = DealerData
end)

RegisterNetEvent('fvm-drugs:client:GotoDealer')
AddEventHandler('fvm-drugs:client:GotoDealer', function(DealerData)
    local ped = PlayerPedId()

    SetEntityCoords(ped, DealerData["coords"]["x"], DealerData["coords"]["y"], DealerData["coords"]["z"])
    FvMain.Functions.Notify('You have been teleported : '.. DealerData["name"] .. ' Good luck!', 'success')
end)