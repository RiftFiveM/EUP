...
FVMClothing.ResetItemTexture = function(obj, category) {
    ...
    $.post('https://fvm-clothing/updateSkin', JSON.stringify({
...
$(document).on('click', '.clothing-menu-option-item-right', function(e){
    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            $.post('https://fvm-clothing/TrackerError');
            return
        } else {
            if (clothingCategory == "model") {
                $(inputElem).val(newValue);
                $.post('https://fvm-clothing/setCurrentPed', JSON.stringify({ped: newValue}), function(model){
                    $("#current-model").html("<p>"+model+"</p>")
                });
                canChange = true;
                FVMClothing.ResetValues()
                ...
                if (buttonType == "item") {
                    FVMClothing.ResetItemTexture(this, clothingCategory);
                }
            } else {
                if (buttonType == "item") {
...
$(document).on('change', '.item-number', function(){
    ...
    if (hasTracker && clothingCategory == "accessory") {
        ...
        if (clothingCategory == "accessory" && inputVal == 13) {
            ...
            $.post('https://fvm-clothing/updateSkinOnInput', JSON.stringify({
                clothingType: clothingCategory,
...
$(document).on('click', '.clothing-menu-header-camera-btn', function(e){
    ...
    if (selectedCam == null) {
        ...
        $.post('https://fvm-clothing/setupCam', JSON.stringify({
            value: camValue
        }));
        selectedCam = this;
    } else {
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 68: // D
            $.post('https://fvm-clothing/rotateRight');
            break;
        case 65: // A
            $.post('https://fvm-clothing/rotateLeft');
            break;
        case 38: // UP
            ChangeUp();
            break;
...
FVMClothing.ToggleChange = function(bool) {
    canChange = bool;
}
...
$(document).ready(function(){
    window.addEventListener('message', function(event) {
        switch(event.data.action) {
            case "open":
                FVMClothing.Open(event.data);
                break;
            case "close":
                FVMClothing.Close();
                break;
            case "updateMax":
                FVMClothing.SetMaxValues(event.data.maxValues);
                break;
            case "reloadMyOutfits":
                FVMClothing.ReloadOutfits(event.data.outfits);
                break;
            case "toggleChange":
                FVMClothing.ToggleChange(event.data.allow);
                break;
            case "ResetValues":
                FVMClothing.ResetValues();
                break;
...
FVMClothing.ReloadOutfits = function(outfits) {
...
$(document).on('click', "#save-menu", function(e){
    e.preventDefault();
    FVMClothing.Close();
    $.post('https://fvm-clothing/saveClothing');
});

$(document).on('click', "#cancel-menu", function(e){
    e.preventDefault();
    FVMClothing.Close();
    $.post('https://fvm-clothing/resetOutfit');
});

FVMClothing.SetCurrentValues = function(clothingValues) {
...
FVMClothing.Open = function(data) {
    ...
    FVMClothing.SetMaxValues(data.maxValues);
    $(".clothing-menu-header").html("");
    FVMClothing.SetCurrentValues(data.currentClothing);
...
$(document).on('click', '.clothing-menu-outfit-option-button', function(e){
    ...
    $.post('https://fvm-clothing/selectOutfit', JSON.stringify({
...
$(document).on('click', '.clothing-menu-myOutfit-option-button-remove', function(e){
    ...
    $.post('https://fvm-clothing/removeOutfit', JSON.stringify({
        outfitData: outfitData.skin,
        outfitName: outfitData.outfitname,
        outfitId: outfitData.outfitId,
    }));
...
FVMClothing.Close = function() {
    $.post('https://fvm-clothing/close');
...
FVMClothing.SetMaxValues = function(maxValues) {
...
FVMClothing.ResetValues = function() {
...
$(document).on('click', '#save-outfit-save', function(e){
    ...
    $.post('https://fvm-clothing/saveOutfit', JSON.stringify({
...
$(document).on('click', '.change-camera-button', function(e){
    ...
    $.post('https://fvm-clothing/rotateCam', JSON.stringify({
        type: rotationType
    }))
});
