...
CreateThread(function()
    while FvMain.Functions.GetPlayerData().job == nil do
        Wait(1)
    end

    currentJob = FvMain.Functions.GetPlayerData().job.name
end)
...
AddEventHandler('dispatch:clNotify', function(pNotificationData)
    if pNotificationData ~= nil and FvMain and FvMain.Functions.GetPlayerData().job then
        if pNotificationData.recipientList then
            for key, value in pairs(pNotificationData.recipientList) do
                if key == currentJob and value and not disableNotifications and FvMain.Functions.GetPlayerData().job.onduty then
...
AddEventHandler('dispatch:toggleNotifications', function(state)
    ...
    if state == "on" then
        ...
        TriggerEvent('FvMain:Notify', "Dispatch Enabled.")
    elseif state == "off" then
        disableNotifications = true
        disableNotificationSounds = true
        TriggerEvent('FvMain:Notify', "Dispatch Disabled.", 'error')
    elseif state == "mute" then
        disableNotifications = false
        disableNotificationSounds = true
        TriggerEvent('FvMain:Notify', "Dispatch Muted.")
    else
        TriggerEvent('FvMain:Notify', "You need to type in 'on', 'off' or 'mute'.", 'error')
    end
end)
...
RegisterNetEvent('dispatch:chopchop')
AddEventHandler('dispatch:chopchop', function(targetVehicle)
    if CallChance() then
        local street1 = GetStreetAndZone()
        local gender = IsPedMale(PlayerPedId())
        local origin = GetEntityCoords(PlayerPedId())
...
AddEventHandler('dispatch:officerDown', function()
    if currentJob == 'police' or currentJob == 'sheriff' then
...
AddEventHandler('dispatch:emsDown', function()
    if currentJob == 'ambulance' or currentJob == 'doctor' then
		local pos = GetEntityCoords(PlayerPedId(),  true)
		TriggerServerEvent("dispatch:svNotify", {
			dispatchCode = "10-13B",
            firstStreet = GetStreetAndZone(),
            callSign = callsignal .. ' | ' .. playerData['charinfo']['firstname']:sub(1,1) .. '. ' .. playerData['charinfo']['lastname'],
			cid = FvMain.Functions.GetPlayerData()['cid'],
			origin = {
				x = pos.x,
				y = pos.y,
				z = pos.z
...
RegisterNetEvent('dispatch:burglary')
AddEventHandler('dispatch:burglary', function()
    local eventId = uuid()
    local dispatchCode = "10-31"
    local street1 = GetStreetAndZone()
    local gender = IsPedMale(PlayerPedId())
    local plyPos = GetEntityCoords(PlayerPedId(), true)
    TriggerServerEvent('dispatch:svNotify', {
...
RegisterNetEvent('dispatch:sus:situation')
AddEventHandler('dispatch:sus:situation', function()
    local eventId = uuid()
...
RegisterNetEvent('dispatch:weapontrafficking')
AddEventHandler('dispatch:weapontrafficking', function()
    local eventId = uuid()
    local dispatchCode = "10-64"
    local street1 = GetStreetAndZone()
    local gender = IsPedMale(PlayerPedId())
...
AddEventHandler('dispatch:fight', function()
    if (currentJob ~= "police" or currentJob ~= "sheriff") and CallChance() then
        local street1 = GetStreetAndZone()
        local gender = IsPedMale(PlayerPedId())
...
AddEventHandler('police:camera', function(id)
    if currentJob == "police" or currentJob == "sheriff" then
        TriggerEvent("chatMessage", "DISPATCH", "warning", "Security Camera number " .. id .. " has been triggered.")
    end
end)
...
CreateThread(function()
	while true do
        Wait(1)
        if (IsControlJustReleased(0, 344) and (currentJob == "police" or currentJob == "sheriff"  or currentJob == "ambulance" or currentJob == "reporter") and not FvMain.Functions.GetPlayerData().metadata["isdead"]) then
