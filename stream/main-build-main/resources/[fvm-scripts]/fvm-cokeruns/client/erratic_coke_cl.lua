...
Citizen.CreateThread(function()
	while FvMain == nil do TriggerEvent("FvMain:GetObject", function(obj) FvMain = obj end)   Wait(0) end
    FvMain.Functions.TriggerCallback('coke:processcoords', function(servercoords)
...
Citizen.CreateThread(function()
	while FvMain == nil do TriggerEvent("FvMain:GetObject", function(obj) FvMain = obj end) Wait(0) end
    FvMain.Functions.TriggerCallback('coke:startcoords', function(servercoords)
        coord = servercoords
	end)
end)

RegisterNetEvent("fvm-cokeruns:SetJobStatus")
AddEventHandler("fvm-cokeruns:SetJobStatus", function(status)
	isJobStarted = status
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if not isJobStarted and dist <= 3 and not started then
			DrawText3Ds(coord.x, coord.y, coord.z, '[~g~E~w~] - Start Coke Run [~r~$5,000~w~]')	
			if not started then
				if dist <= 1 then
					if IsControlJustPressed(1, 51) and not started then
						FvMain.Functions.TriggerCallback('coke:pay', function(success)
							if success then
								main()
								TriggerServerEvent("fvm-cokeruns:SetJobStats", true)
							else
		...
		elseif isJobStarted and dist <= 3 then
...
function main()
	...
	FvMain.Functions.Notify("Head to the airfield!", "primary")
	...
	Citizen.CreateThread(function()
		while enroute do
			...
			if disttocoord <= 100 and not Config.landPlane then
				if not IsAnyVehicleNearPoint(location.stationary.x,location.stationary.y,location.stationary.z, 6.0) then
					planeGround()
					enroute = false
				end
...
function planeGround()
	...
	FvMain.Functions.Notify("You need fuel..", "error")
...
function planeFly()
	...
	FvMain.Functions.Notify("Wait for the plane..", "primary")
		...
		while fueling do
			...
			if disttocoord <= 3 then
				...
				if IsControlJustPressed(1, 51) then
					FvMain.Functions.Progressbar("jerry_pickup", "Picking up jerry can..", math.random(5000, 10000), false, true, {
						...
						TriggerEvent("inventory:client:ItemBox", FvMain.Shared.Items['jerry_can'], "add")
						FreezeEntityPosition(player, false)
						TriggerEvent('FvMain:Notify', "Fuel the plane!")
						plane(fuel)
						fueling = false
						dodo = true
					end, function() -- Cancel
	...
	Citizen.CreateThread(function()
		while dodo do
			...
			if disttocoord <= 5 then
				...
				if IsControlJustPressed(1, 51) then
					FvMain.Functions.TriggerCallback('coke:jerrycheck', function(success)
						if success then
							TriggerServerEvent('coke:RemoveJerry')
							FvMain.Functions.Progressbar("plane_refuel", "You are fueling the plane..", math.random(5000, 10000), false, true, {
								disableMovement = true,
								disableCarMovement = true,
								disableMouse = false,
								disableCombat = true,
								...
								dodo = false
								delivering = true
								FvMain.Functions.Notify("Finished refueling!", "success")
								...
								delivery()
								started = false
								TriggerServerEvent("fvm-cokeruns:SetJobStats", false)
							end, function() -- Cancel
							...
							isProcessing = false
							FvMain.Functions.Notify("You need a jerry can..", "error")
						end
					end)
				end
			else
	...
	while checkPlane do
		...
		if DoesEntityExist(airplane) then
			if GetVehicleEngineHealth(airplane) < 0 then
				FvMain.Functions.Notify("The vehicle was damaged.. You failed the run", "error")
				TriggerServerEvent('coke:updateTable', false)
				checkPlane = false
				inUse = false
				RemoveBlip(blip)
	...
	Citizen.CreateThread(function()
		while delivering do
			...
			if disttocoord <= 20 then
				...
				if IsControlJustPressed(1, 51) then
					FvMain.Functions.Progressbar("package_pickup", "Picking up package..", math.random(5000, 10000), false, true, {
						disableMovement = true,
						disableCarMovement = true,
						disableMouse = false,
						disableCombat = true,
						...
						delivering = false
						FvMain.Functions.Notify("You picked up a delivery!", "success")
...
function final()
	FvMain.Functions.Notify("Deliver the plane back to the airfield!", "success")
		...
		while hangar do
			...
			if disttocoord <= 5 then
				...
				if IsControlJustPressed(1, 51) then
					hangar = false
					FreezeEntityPosition(airplane, true)
					FvMain.Functions.Progressbar("park_plane", "Storing the plane..", math.random(5000, 10000), false, true, {
						disableMovement = true,
						disableCarMovement = true,
						disableMouse = false,
						disableCombat = true,
						...
						TriggerEvent("inventory:client:ItemBox", FvMain.Shared.Items['coke_brick'], "add")
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if dist <= 3 and not isProcessing then
			...
			if IsControlJustPressed(1, 51) then		
				isProcessing = true
				FvMain.Functions.TriggerCallback('coke:process', function(success)
					if success then
						SetEntityHeading(player, 232.84)
						FvMain.Functions.Progressbar("coke_breakdown", "Breaking down the coke brick..", math.random(10000, 20000), false, true, {
							disableMovement = true,
							disableCarMovement = true,
							disableMouse = false,
							disableCombat = true,
							...
							TriggerEvent("inventory:client:ItemBox", FvMain.Shared.Items['coke_brick'], "remove")
						...
						TriggerEvent('FvMain:Notify', "You need a coke brick!")
						isProcessing = false
					end
				end)
			end
