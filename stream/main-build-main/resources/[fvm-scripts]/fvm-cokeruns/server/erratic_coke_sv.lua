...
FvMain.Functions.CreateCallback('coke:processcoords', function(source, cb)
    cb(hiddenprocess)
end)

FvMain.Functions.CreateCallback('coke:startcoords', function(source, cb)
    cb(hiddenstart)
end)

FvMain.Functions.CreateCallback('coke:process', function(source, cb)
...
FvMain.Functions.CreateCallback('coke:jerrycheck', function(source, cb)
    local Player = FvMain.Functions.GetPlayer(source)
...
RegisterNetEvent("fvm-cokeruns:SetJobStats")
AddEventHandler("fvm-cokeruns:SetJobStats", function(status)
    isJobStarted = status
    TriggerClientEvent("fvm-cokeruns:SetJobStatus", -1, isJobStarted)
end)


FvMain.Functions.CreateCallback('coke:pay', function(source, cb)
	local src = source
...
FvMain.Functions.CreateUseableItem("coke_small_brick", function(source, item)
    local src = source
    local Player = FvMain.Functions.GetPlayer(src)
	if Player.Functions.RemoveItem(item.name, 1) then
        Player.Functions.AddItem("cokebaggy", math.random(20, 30))
        TriggerClientEvent('inventory:client:ItemBox', src, FvMain.Shared.Items["cokebaggy"], 'add')
    end
end)
...
AddEventHandler("coke:processed", function(x,y,z)
    ...
    TriggerClientEvent('inventory:client:ItemBox', src, FvMain.Shared.Items["coke_small_brick"], 'add')
end)
