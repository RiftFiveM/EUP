...
-- Get permissions --
FvMain.Functions.CreateCallback('fvm-anticheat:server:GetPermissions', function(source, cb)
...
RegisterNetEvent('fvm-anticheat:server:banPlayer')
AddEventHandler('fvm-anticheat:server:banPlayer', function(reason)
    if EventTriggered then
        local src = source
        local pData = FvMain.Functions.GetPlayer(src)
        TriggerClientEvent('chatMessage', -1, "Anti-Cheat", "error", GetPlayerName(src).." was banned for: " ..reason )
        FvMain.Functions.ExecuteSql(false, "INSERT INTO `bans` (`name`, `steam`, `license`, `discord`,`ip`, `reason`, `expire`, `bannedby`) VALUES ('"..GetPlayerName(src).."', 'nil', '"..pData.PlayerData.license.."', '"..GetPlayerIdentifiers(src)[2].."', 'nil', '"..reason.."', 2145913200, '"..GetPlayerName(src).."')")
        DropPlayer(src, "You have been flagged to much, for more information join our discord: discord URL")
    else
        local time = os.date("*t")
        local Player = FvMain.Functions.GetPlayer(source)
        if Player ~= nil then
            print("///----------------------------- TIME ".. time.hour ..":".. time.min .."------------------------------------------///")
            FvMain.ShowSuccess(GetCurrentResourceName(), "ban event was triggered by some player, there is a list of players on the server")
            for _, playerId in ipairs(GetPlayers()) do
                local name = GetPlayerName(playerId)
                local license = FvMain.Functions.GetIdentifier(playerId, 'license')
                print(("Player %s with id %i and license %s"):format(name, playerId, license)) 
                print("///-----------------------------------------------------------------------///")
                TriggerEvent("fvm-log:server:CreateLog", "anticheat", "Ban Triggered by Player", "red", "Event was triggered but wasn't enabled, check server console for more info.") 
            end
        end
    end
end)

RegisterNetEvent('fvm-anticheat:server:IsPlayerBanAble')
AddEventHandler('fvm-anticheat:server:IsPlayerBanAble', function()
    EventTriggered = true
    Wait(2000)
    EventTriggered = false
end)
...
function NonRegisteredEventCalled(CalledEvent, source)
    TriggerClientEvent("fvm-anticheat:client:NonRegisteredEventCalled", source, "Cheating", CalledEvent)
end
...
FvMain.Functions.CreateCallback('fvm-anticheat:server:HasWeaponInInventory', function(source, cb, WeaponInfo)
    local src = source
