...
function GetCurrentProject()
    FvMain.Functions.TriggerCallback('fvm-builderjob:server:GetCurrentProject', function(BuilderConfig)
        ...
        if Config.CurrentProject ~= 0 then
                    ...
                    if TaskData ~= nil then
                        ...
                        if TaskData.completed == TaskData.total then
                            ...
                            if IsControlJustPressed(0, Keys["G"]) then
                                print("cancer project ended")
                                TriggerServerEvent('fvm-builderjob:server:FinishProject')
                            end
                        end
            ...
            for k, v in pairs(Config.Projects[Config.CurrentProject].ProjectLocations["tasks"]) do
                if not v.completed or not v.IsBusy then
                    local TakeDistance = #(pos - vector3(v.coords.x, v.coords.y, v.coords.z))
...
function DoTask()
    ...
    local CountDown = 5
    TriggerServerEvent('fvm-builderjob:server:SetTaskState', BuilderData.CurrentTask, true, false)

    if TaskData.type == "hammer" then
        ExecuteCommand("/a hammer")
        while CountDown ~= 0 do
            CountDown = CountDown - 1
            
            Citizen.Wait(1000)
        end
        TriggerServerEvent('fvm-builderjob:server:SetTaskState', BuilderData.CurrentTask, true, true)
        ClearPedTasks(ped)
    end
end

RegisterNetEvent('fvm-builderjob:client:SetTaskState')
AddEventHandler('fvm-builderjob:client:SetTaskState', function(Task, IsBusy, IsCompleted)
    ...
    if IsCompleted then
        print(Config.Projects[Config.CurrentProject].ProjectLocations["tasks"][Task].label.." is completed!")
    end
end)

RegisterNetEvent('fvm-builderjob:client:FinishProject')
AddEventHandler('fvm-builderjob:client:FinishProject', function(BuilderConfig)
    Config = BuilderConfig
end)