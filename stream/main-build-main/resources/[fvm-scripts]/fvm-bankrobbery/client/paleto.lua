...
Citizen.CreateThread(function()
    Citizen.Wait(2000)
    local requiredItems = {
        [1] = {name = FvMain.Shared.Items["security_card_01"]["name"], image = FvMain.Shared.Items["security_card_01"]["image"]},
    }
                                    ...
                                    if lockerDist < 0.5 then
                                        ...
                                        if IsControlJustPressed(0, 38) then
                                            if CurrentCops >= Config.MinimumPaletoPolice then
                                                ...
                                                FvMain.Functions.Notify('Minimum Of '..Config.MinimumPaletoPolice..' Police Needed', "error")
                                            end
                                        end
                                    end
                                end
...
RegisterNetEvent('fvm-bankrobbery:UseBankcardA')
AddEventHandler('fvm-bankrobbery:UseBankcardA', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    local dist = #(pos - vector3(Config.BigBanks["paleto"]["coords"]["x"], Config.BigBanks["paleto"]["coords"]["y"],Config.BigBanks["paleto"]["coords"]["z"]))
        ...
        FvMain.Functions.TriggerCallback('fvm-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if CurrentCops >= Config.MinimumPaletoPolice then
                    if not Config.BigBanks["paleto"]["isOpened"] then 
                        TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                        FvMain.Functions.Progressbar("security_pass", "Validitating card..", math.random(5000, 10000), false, true, {
                            ...
                            TriggerServerEvent('fvm-bankrobbery:server:setBankState', "paleto", true)
                            TriggerServerEvent("FvMain:Server:RemoveItem", "security_card_01", 1)
                            TriggerServerEvent('fvm-doorlock:server:updateState', 67, false)
                            if not copsCalled then
                                ...
                                if Config.BigBanks["paleto"]["alarm"] then
                                    TriggerServerEvent("fvm-bankrobbery:server:callCops", "paleto", 0, streetLabel, pos)
                    ...
                    FvMain.Functions.Notify('Not enough police.. ('..Config.MinimumPaletoPolice..' needed)', "error")
                end
            else
