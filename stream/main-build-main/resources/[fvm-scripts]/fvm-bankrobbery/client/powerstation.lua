        ...
        if dist < 1.5 then
            if CurrentCops >= Config.MinimumThermitePolice then
                if not Config.PowerStations[closestStation].hit then
                    ...
                    FvMain.Functions.Notify("It seems that the fuses have burned out..", "error")
                end
            else
                FvMain.Functions.Notify("Not enough police (2 needed)", "error")
            end
        end
...
RegisterNetEvent('fvm-bankrobbery:client:SetStationStatus')
AddEventHandler('fvm-bankrobbery:client:SetStationStatus', function(key, isHit)
...
RegisterNUICallback('thermitefailed', function()
    ...
    TriggerServerEvent("FvMain:Server:RemoveItem", "thermite", 1)
    TriggerEvent('inventory:client:ItemBox', FvMain.Shared.Items["thermite"], "remove")
...
RegisterNUICallback('thermitesuccess', function()
    ...
    local coords = GetEntityCoords(PlayerPedId())
    while time > 0 do 
        FvMain.Functions.Notify("Burning in " .. time .. "..")
        Citizen.Wait(1000)
        time = time - 1
    end
    local randTime = math.random(10000, 15000)
    ...
    if currentStation ~= 0 then
        FvMain.Functions.Notify("The fuses are broken", "success")
        TriggerEvent("dispatch:powerStation")
        TriggerServerEvent("fvm-bankrobbery:server:SetStationStatus", currentStation, true)
    elseif currentGate ~= 0 then
        FvMain.Functions.Notify("The door is burned open", "success")
        TriggerServerEvent('fvm-doorlock:server:updateState', currentGate, false)
        currentGate = 0
    end
end)
