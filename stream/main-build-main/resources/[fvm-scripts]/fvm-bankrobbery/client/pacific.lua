...
Citizen.CreateThread(function()
    ...
    local requiredItems3 = {
        [1] = {name = FvMain.Shared.Items["thermite"]["name"], image = FvMain.Shared.Items["thermite"]["image"]},
    }
    ...
    local requiredItems = {
        [1] = {name = FvMain.Shared.Items["security_card_02"]["name"], image = FvMain.Shared.Items["security_card_02"]["image"]},
    }
    while true do
                                ...
                                if lockerDist < 0.5 then
                                    ...
                                    if IsControlJustPressed(0, 38) then
                                        if CurrentCops >= Config.MinimumPacificPolice then
                                            ...
                                            FvMain.Functions.Notify('Not enough police.. ('..Config.MinimumPacificPolice..' needed)', "error")
                                        end
                                    end
                                end
                            end
    ...
    if dist < 1.5 then
        FvMain.Functions.TriggerCallback('fvm-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                ...
                if dist < 1.5 then
                    if CurrentCops >= Config.MinimumPacificPolice then
                        if not Config.BigBanks["pacific"]["isOpened"] then 
                                    ...
                                    FvMain.Functions.Progressbar("hack_gate", "Connecting electronic kit..", math.random(5000, 10000), false, true, {
                                        ...
                                        if not copsCalled then
                                            ...
                                            if Config.BigBanks["pacific"]["alarm"] then
                                                TriggerServerEvent("fvm-bankrobbery:server:callCops", "pacific", 0, streetLabel, pos)
                            ...
                            FvMain.Functions.Notify("Looks like the bank is already open", "error")
                        end
                    else
                        FvMain.Functions.Notify('Not enough police.. ('..Config.MinimumPacificPolice..' needed)', "error")
                    end
                ...
                FvMain.Functions.Notify("The security lock is active, opening the door is currently not possible.", "error", 5500)
            end
        end)
    end
end)

RegisterNetEvent('fvm-bankrobbery:UseBankcardB')
AddEventHandler('fvm-bankrobbery:UseBankcardB', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    local dist = #(pos - vector3(Config.BigBanks["pacific"]["coords"][1]["x"], Config.BigBanks["pacific"]["coords"][1]["y"],Config.BigBanks["pacific"]["coords"][1]["z"]))
        ...
        FvMain.Functions.TriggerCallback('fvm-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if CurrentCops >= Config.MinimumPacificPolice then
                    if not Config.BigBanks["pacific"]["isOpened"] then 
                        TriggerEvent('inventory:client:requiredItems', requiredItems2, false)
                        FvMain.Functions.Progressbar("security_pass", "Adjust to validate...", math.random(5000, 10000), false, true, {
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
                            ...
                            StopAnimTask(PlayerPedId(), "anim@gangops@facility@servers@", "hotwire", 1.0)
                            TriggerServerEvent('fvm-doorlock:server:updateState', 2, false)
                            TriggerServerEvent("FvMain:Server:RemoveItem", "security_card_02", 1)
                            if not copsCalled then
                                local s1, s2 = Citizen.InvokeNative(0x2EB41072B4C1E4C0, pos.x, pos.y, pos.z, Citizen.PointerValueInt(), Citizen.PointerValueInt())
                                local street1 = GetStreetNameFromHashKey(s1)
                                local street2 = GetStreetNameFromHashKey(s2)
...
function OnHackPacificDone(success, timeremaining)
    if success then
        ...
        TriggerServerEvent('fvm-bankrobbery:server:setBankState', "pacific", true)
    else
		TriggerEvent('mhacking:hide')
	end
end
