...
Citizen.CreateThread(function()
    while true do
        ...
        if blackoutActive then
            TriggerEvent("fvm-weathersync:server:toggleBlackout")
            TriggerClientEvent("police:client:EnableAllCameras", -1)
            TriggerClientEvent("fvm-bankrobbery:client:enableAllBankSecurity", -1)
...
RegisterServerEvent('fvm-bankrobbery:server:setBankState')
AddEventHandler('fvm-bankrobbery:server:setBankState', function(bankId, state)
    if bankId == "paleto" then
        if not robberyBusy then
            Config.BigBanks["paleto"]["isOpened"] = state
            TriggerClientEvent('fvm-bankrobbery:client:setBankState', -1, bankId, state)
            TriggerEvent('fvm-scoreboard:server:SetActivityBusy', "bankrobbery", true)
            TriggerEvent('fvm-bankrobbery:server:setTimeout')
        end
    elseif bankId == "pacific" then
        if not robberyBusy then
            Config.BigBanks["pacific"]["isOpened"] = state
            TriggerClientEvent('fvm-bankrobbery:client:setBankState', -1, bankId, state)
            TriggerEvent('fvm-scoreboard:server:SetActivityBusy', "pacific", true)
            TriggerEvent('fvm-bankrobbery:server:setTimeout')
        end
    else
        if not robberyBusy then
            ...
            TriggerClientEvent('fvm-bankrobbery:client:setBankState', -1, bankId, state)
            TriggerEvent('fvm-banking:server:SetBankClosed', bankId, true)
            TriggerEvent('fvm-scoreboard:server:SetActivityBusy', "bankrobbery", true)
            TriggerEvent('fvm-bankrobbery:server:SetSmallbankTimeout', bankId)
        end
    end
    robberyBusy = true
end)

RegisterServerEvent('fvm-bankrobbery:server:setLockerState')
AddEventHandler('fvm-bankrobbery:server:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "paleto" then
    ...
    TriggerClientEvent('fvm-bankrobbery:client:setLockerState', -1, bankId, lockerId, state, bool)
end)

RegisterServerEvent('fvm-bankrobbery:server:recieveItem')
AddEventHandler('fvm-bankrobbery:server:recieveItem', function(type)
    local src = source
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                if Config.RewardTypes[itemType].type == "item" then
                    ...
                    TriggerClientEvent('inventory:client:ItemBox', src, FvMain.Shared.Items[item.item], "add")
                elseif Config.RewardTypes[itemType].type == "money" then
                    ply.Functions.AddItem('markedbills', Config.MarkedBillsSmallBank)
                    TriggerClientEvent('inventory:client:ItemBox', src, FvMain.Shared.Items['markedbills'], "add")
                end
            else
                ply.Functions.AddItem('security_card_01', 1)
                TriggerClientEvent('inventory:client:ItemBox', src, FvMain.Shared.Items['security_card_01'], "add")
            end
        else
            ply.Functions.AddItem('weapon_stungun', 1)
            TriggerClientEvent('inventory:client:ItemBox', src, FvMain.Shared.Items['weapon_stungun'], "add")
        end
    elseif type == "paleto" then
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, FvMain.Shared.Items['security_card_02'], "add")
            end
        else
            ply.Functions.AddItem('weapon_vintagepistol', 1)
            TriggerClientEvent('inventory:client:ItemBox', src, FvMain.Shared.Items['weapon_vintagepistol'], "add")
        end
    elseif type == "pacific" then
        ...
        if WeaponChance ~= odd1 or WeaponChance ~= odd2 then
            if tier ~= 4 then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, FvMain.Shared.Items['cryptostick'], "add")
            end
        else
            ...
            if chance == odd then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, FvMain.Shared.Items['weapon_microsmg'], "add")
            else
                ply.Functions.AddItem('weapon_minismg', 1)
                TriggerClientEvent('inventory:client:ItemBox', src, FvMain.Shared.Items['weapon_minismg'], "add")
            end
        end
    end
end)

FvMain.Functions.CreateCallback('fvm-bankrobbery:server:isRobberyActive', function(source, cb)
    cb(robberyBusy)
end)

FvMain.Functions.CreateCallback('fvm-bankrobbery:server:GetConfig', function(source, cb)
    cb(Config)
end)

RegisterServerEvent('fvm-bankrobbery:server:setTimeout')
AddEventHandler('fvm-bankrobbery:server:setTimeout', function()
    if not robberyBusy then
        if not timeOut then
            timeOut = true
            Citizen.CreateThread(function()
                ...
                timeOut = false
                robberyBusy = false
                TriggerEvent('fvm-scoreboard:server:SetActivityBusy', "bankrobbery", false)
                TriggerEvent('fvm-scoreboard:server:SetActivityBusy', "pacific", false)
                ...
                TriggerClientEvent('fvm-bankrobbery:client:ClearTimeoutDoors', -1)
...
RegisterServerEvent('fvm-bankrobbery:server:SetSmallbankTimeout')
AddEventHandler('fvm-bankrobbery:server:SetSmallbankTimeout', function(BankId)
    if not robberyBusy then
        if not timeOut then
            timeOut = true
            Citizen.CreateThread(function()
                ...
                timeOut = false
                robberyBusy = false
            	TriggerClientEvent('fvm-bankrobbery:client:ResetFleecaLockers', -1, BankId)
            	TriggerEvent('fvm-banking:server:SetBankClosed', BankId, false)
            end)
	    end
    end
end)

RegisterServerEvent('fvm-bankrobbery:server:callCops')
AddEventHandler('fvm-bankrobbery:server:callCops', function(type, bank, streetLabel, coords)
    local cameraId = 4
    if type == "small" then
...
RegisterServerEvent('fvm-bankrobbery:server:SetStationStatus')
AddEventHandler('fvm-bankrobbery:server:SetStationStatus', function(key, isHit)
    Config.PowerStations[key].hit = isHit
    TriggerClientEvent("fvm-bankrobbery:client:SetStationStatus", -1, key, isHit)
    if AllStationsHit() then
        TriggerEvent("fvm-weathersync:server:toggleBlackout")
        TriggerClientEvent("police:client:DisableAllCameras", -1)
        TriggerClientEvent("fvm-bankrobbery:client:disableAllBankSecurity", -1)
        blackoutActive = true
    else
        CheckStationHits()
    end
...
function CheckStationHits()
    ...
    if Config.PowerStations[11].hit and Config.PowerStations[1].hit and Config.PowerStations[2].hit then
        ...
        TriggerClientEvent("fvm-bankrobbery:client:BankSecurity", 1, false)
        TriggerClientEvent("police:client:SetCamera", -1, 22, false)
        TriggerClientEvent("fvm-bankrobbery:client:BankSecurity", 2, false)
    end
    if Config.PowerStations[8].hit and Config.PowerStations[4].hit and Config.PowerStations[5].hit and Config.PowerStations[6].hit then
        ...
        TriggerClientEvent("fvm-bankrobbery:client:BankSecurity", 3, false)
    end
    if Config.PowerStations[12].hit and Config.PowerStations[13].hit then
        TriggerClientEvent("police:client:SetCamera", -1, 24, false)
        TriggerClientEvent("fvm-bankrobbery:client:BankSecurity", 4, false)
        TriggerClientEvent("police:client:SetCamera", -1, 25, false)
        TriggerClientEvent("fvm-bankrobbery:client:BankSecurity", 5, false)
    end
end
...
FvMain.Functions.CreateUseableItem("thermite", function(source, item)
    local Player = FvMain.Functions.GetPlayer(source)
	if Player.Functions.GetItemByName('lighter') ~= nil then
        TriggerClientEvent("thermite:UseThermite", source)
    else
        TriggerClientEvent('FvMain:Notify', source, "You are missing something to light the thermite..", "error")
    end
end)

FvMain.Functions.CreateUseableItem("security_card_01", function(source, item)
    local Player = FvMain.Functions.GetPlayer(source)
	if Player.Functions.GetItemByName('security_card_01') ~= nil then
        TriggerClientEvent("fvm-bankrobbery:UseBankcardA", source)
    end
end)

FvMain.Functions.CreateUseableItem("security_card_02", function(source, item)
    local Player = FvMain.Functions.GetPlayer(source)
	if Player.Functions.GetItemByName('security_card_02') ~= nil then
        TriggerClientEvent("fvm-bankrobbery:UseBankcardB", source)
    end
end)